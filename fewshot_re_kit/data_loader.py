...
def collate_fn(data):
    ...
    for i in range(len(support_sets)):
        for k in support_sets[i]:   # the k is one of the dict.keys()
...
def get_loader(name, encoder, N, K, Q, batch_size, 
        num_workers=8, collate_fn=collate_fn, na_rate=0, root='./data', is_orsoftmax=False):
    if is_orsoftmax:
        dataset = FewRelDatasetForNormalSoftmax(name, encoder, N, K, Q, na_rate, root)
    else:
